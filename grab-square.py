...
def hamle_yaptirma(hamle_liste,sekil_sayi,sekil_harf,hamle_satir,hamle_sutun):
    if hamle_liste[2] in ["D", "B"]: #doğu-batı hamle kontrolleri
        ...
        if hamle_liste[2] == "D":
            hamle_sutun += 1
    elif hamle_liste[2] in ["K", "G"]: #kuzey-güney hamle kontrolleri
        birinci_parametre_say = -2
        for x in sekil_sayi:
            birinci_parametre_say += 2
            if x == hamle_liste[0]:
